...
function Done(){
    ...
    function blobToDataUrl(){//form태그는 다른 서버로 전송x -> axios는 가능
        ...
        reader.onload=function(event){
            ...
            if (dataUrl){
                ...
                form.setAttribute("action", `https://admin.goodde.kr:3010/call/message/${id}/imgsubmit`); //요청 보낼 주소 (수정 필요) `goodde.kr:3010/call/message/:${id}/imgsubmit`
    ...
    function blobToDataUrl_axios(){//혹시 form태그 동작하지 않으면 axios로 해보세용..!!!
        ...
        reader.onload=function(event){
            ...
            if (dataUrl){
                ...
                axios.post(`https://admin.goodde.kr:3010/call/message/${id}/imgsubmit`, {//정보 전달할 페이지
                    text:text.value,
                    dataUrl:dataUrl
                })
    ...
    function text_axios(){//axios 써서 서버로 정보 보내기
        var text=document.getElementById('text');
        if (text){
            axios.post(`https://admin.goodde.kr:3010/call/message/${id}/textsubmit`, {//정보 전달할 페이지
                text:text.value,
            })
            .then((res)=>{//axios.post 성공하면
                console.log(res);
